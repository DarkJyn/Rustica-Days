...
/**
 * Hạt giống củ dền – đơn giản chỉ chứa thông tin metadata
 */
public class RadishSeed extends Seed {

    // Constructor với tham số tên và mô tả từ người gọi
    public RadishSeed(String name, String description, int price, TextureRegion textureOverride) {
        super(
            name,                                        // Tên
            description,                                 // Mô tả
            price,                                       // Giá mua
    ...
    private static TextureRegion loadDefaultTexture() {
        try {
            return new TextureRegion(new Texture(Gdx.files.internal("[Rustica] Asset/Cute_Fantasy/Cute_Fantasy/Crops/RadishSeed.png")));
        } catch (Exception e) {
            System.err.println("Could not load RadishSeed texture: " + e.getMessage());
            return null;
        }
    }
}
