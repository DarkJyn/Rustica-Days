...
public class RiceSeed extends Seed {
    ...
    public RiceSeed(String name, String description, int price, TextureRegion textureOverride) {
        super(
            name,                                        // Tên
            description,                                 // Mô tả
            price,                                       // Giá mua
    ...
    private static TextureRegion loadDefaultTexture() {
        try {
            return new TextureRegion(new Texture(Gdx.files.internal("[Rustica] Asset/Cute_Fantasy/Cute_Fantasy/Crops/RiceSeed.png")));
        } catch (Exception e) {
            System.err.println("Could not load RiceSeed texture: " + e.getMessage());
            return null;
        }
    }
}
