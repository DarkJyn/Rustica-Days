...
/**
 * Cây tỏi - chỉ trồng, tưới và thu hoạch
 */
public class Garlic extends Plant {
    ...
    private static final int FIXED_PRICE = 200;                 // Giá bán cố định
    private static final int FIXED_EXP = 100;

    public Garlic(float x, float y, float width, float height) {
    ...
    protected void initTextures() {
        try {
            Texture sheet = new Texture(Gdx.files.internal("[Rustica] Asset/Cute_Fantasy/Cute_Fantasy/Crops/Garlic.png"));
            int sheetFrames = 7;
            ...
            System.err.println("Could not load Garlic texture: " + e.getMessage());
    ...
    private void createDefaultTexture() {
        ...
        for (GrowthState state : GrowthState.values()) {
            ...
            frames[0] = new TextureRegion(new Texture(Gdx.files.internal("[Rustica] Asset/Cute_Fantasy/Cute_Fantasy/Crops/GarlicSeed.png")));
